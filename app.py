...
app = Flask(__name__)
@app.route('/')
def home(): return "All-in-One Bot is Live & Secure!"
...
if GEMINI_KEY:
    ...
    # استخدمنا latest لضمان عدم ظهور خطأ 404
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("أهلاً بك يا مصطفى! 🤖\nأنا شغال هسي بأمان كامل وبأحدث موديل. أرسل نص أو PDF.")

async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not model:
        await update.message.reply_text("❌ مشكلة في مفتاح Gemini.")
        return
    ...
    try:
        ...
        res_text = response.text if response.text else "لم أستطع توليد رد."
        
        await update.message.reply_text(res_text[:4096])
        
        # تحويل الرد لصوت تلقائياً
